#> entity:load

scoreboard objectives add entity.liquid_mana.life dummy

scoreboard objectives add entity.fire_magic_book_fire.move dummy
scoreboard objectives add entity.fire_magic_book_fire.time dummy

scoreboard objectives add entity.tomato_juice.life dummy

scoreboard objectives add entity.xp_item.life dummy
scoreboard objectives add entity.gold_item.life dummy

scoreboard objectives add entity.attackHitCooltime dummy